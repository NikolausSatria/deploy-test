...
export default async function handler(req, res) {
  try {
    ...
    if (method === "GET") {
      ...
      if (search) {
        sqlQuery += ` WHERE combined.description LIKE ?`;
        countQuery += ` WHERE combined.description LIKE ?`;
        values.push(`%${search}%`);
      }
      ...
      try {
        ...
        const totalItems = allData === "true" ? inventory.length : totalCountResult[0]?.total_count || 0;
